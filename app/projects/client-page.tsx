...
export default function ProjectsClientPage(props: ClientProjectProps) {
  const { data } = useTina({ ...props });
  const { theme } = useLayout();

  return (
    <>
      {data?.projectConnection.edges.map((projectData) => {
        ...
        return (
          <Link
            key={project.id}
            href={`/projects/` + project._sys.breadcrumbs.join("/")}
